...
// array of instructions  44 {instruction,format,OPCODE,funct3,funct 7}
...
int binarySearchString(string x, int n) // search for the index in array instructions through instructions name
{
    int lower = 0;
    int upper = n - 1;
    while (lower <= upper)
...
int binToDec(string n) // converts binary to decimal
{
    string num = n;
    int dec_value = 0;
...
string decToBinary(int decimal, int digits) // ddecimal to binary
{
    bool flag = false;
    if (decimal < 0)
    {
...
string decTohex(int decimal) // decimal to hex
{
    bool flag = false;
    if (decimal < 0)
    {
...
// Removes leading and trailing commas from the string
...
// Removes trailing colon from the string
void remove_colon(string& word)
{
...
string remove(string& word, char character)
{
...
{
    string file_name;
    cout << "please enter name of the file you want to test";
    cin >> file_name;
    program(file_name);
